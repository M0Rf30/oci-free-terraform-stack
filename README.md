...
The workflow resolves the Object Storage namespace automatically from the OCI
API credentials, so it is not a workflow input or repository variable. The
dispatch form only asks for the region and the state bucket. The bucket is
created automatically in the selected region if it does not already exist.
...
The optional `dht_port` variable opens the Bitmagnet DHT crawler's TCP and UDP
port on the public security list. It defaults to `3334`; set it to `0` to
disable both ingress rules.
